export const checkConfig = (chatMode, agentConfig, modelConfig) => {
  ...
  try {
    // Note: environment property is not available in getSystemSetting, use appBaseInfo instead
    if (appBaseInfo.host?.env === "SDK") {
      return [false, "请前往微信客户端扫码打开小程序"];
    }
  } catch (e) {
    // console.log('getSystemSetting 接口失败')
  ...
  if (compareVersions(appBaseInfo.SDKVersion, "3.7.7") < 0) {
    return [false, "当前微信版本太低，使用AI功能。请前往应用商城更新至最新版微信。"];
  }
